...
with col2:
    ...
    prev_return = st.slider(
        "🟡 Previous Year's Nifty Return (%)",
        min_value=-60.0, max_value=80.0, value=15.0, step=0.5,
        help="Last year's actual Nifty 50 return. Markets often mean-revert after extreme years."
    )
...
if st.button("🚀 Predict Market Behaviour", use_container_width=True, type="primary"):
    ...
    input_data = np.array([[inflation, gdp, repo, prev_return / 100.0]])
    prediction = model.predict(input_data)[0]
    pred_pct = prediction * 100

    # Z-score relative to historical average
    z_score = (prediction - mean_ret) / std_ret
    ...
    with col_a:
        st.metric(
            ...
            value=f"{pred_pct:+.2f}%",
            delta="Above historical avg" if pred_pct > mean_ret * 100 else "Below historical avg"
        )
    ...
    if prev_return < -20:
        flags.append("🔄 **Sharp Previous Decline** — markets may see mean reversion / recovery")
    elif prev_return > 40:
        flags.append("🔄 **Strong Previous Rally** — momentum may continue but correction risk rises")
